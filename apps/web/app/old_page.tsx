...
import { LEAGUES, type LeagueKey } from "../src/lib/leagues"; // adjust if your leagues.ts is elsewhere
...
function statusBadge(status?: string) {
  ...
  // label + simple colors
  if (s === "FT" || s === "AET" || s === "PEN")
    return { text: s, bg: "#e8f5e9", border: "#b7e1be" }; // green-ish
  if (s === "NS")
    return { text: "NS", bg: "#e3f2fd", border: "#bbdefb" }; // blue-ish
  if (s === "1H" || s === "2H" || s === "HT" || s === "LIVE")
    return { text: s === "LIVE" ? "LIVE" : s, bg: "#fff3e0", border: "#ffe0b2" }; // orange-ish
  if (s === "PST" || s === "SUSP" || s === "CANC")
    return { text: s, bg: "#ffebee", border: "#ffcdd2" }; // red-ish

  return { text: s || "?", bg: "#f5f5f5", border: "#e0e0e0" }; // neutral
}
...
export default function Page() {
  ...
  const [to, setTo] = useState(toYMD(new Date(today.getTime() + 6 * 24 * 60 * 60 * 1000))); // +6 days
  const [season, setSeason] = useState<number>(() => {
    const y = today.getFullYear();
    const m = today.getMonth() + 1; // 1..12
    // Europe-style season: Aug–May => season year is the start year
    return m < 7 ? y - 1 : y;
  });
  ...
  return (
    <main style={{ padding: 18, maxWidth: 1200, margin: "0 auto", fontFamily: "system-ui, -apple-system, Segoe UI, Roboto" }}>
      ...
      >
        ...
        <button
          onClick={scanAll}
          disabled={scanLoading || fixtures.length === 0}
          style={{
            padding: "9px 12px",
            borderRadius: 10,
            border: "1px solid #ddd",
            background: scanLoading ? "#f3f3f3" : "white",
      ...
      <section style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: 12 }}>
        ...
        {scanTop?.length ? (
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontWeight: 800, marginBottom: 6 }}>League top (from loaded fixtures)</div>
            <div style={{ display: "grid", gap: 8 }}>
              {scanTop.map((p, i) => (
                <button
                  key={i}
